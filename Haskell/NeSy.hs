...
type NeSyFrameworkTransformation t1 s1 omega1 t2 s2 omega2 =
  forall a.
  (Ord a) =>
  Interpretation t1 s1 omega1 a ->
  Interpretation t2 s2 omega2 a

-- argmax NeSy transformation
maximalValues ::
  (Num prob, Ord prob, Eq prob, Ord a) =>
  Dist.T prob a ->
  SM.Set a
...
argmax ::
  (Num prob, Ord prob, Eq prob) =>
  NeSyFrameworkTransformation
    (Dist.T prob)
    []
    Bool
    SM.Set
...
evalT ::
  (NeSyFramework t s omega) =>
  Interpretation t s omega a ->
  Valuation a ->
  Term ->
  a
...
evalF i val (Pred p ts) = return $ p_sem $ map (evalT i val) ts
  where
    p_sem = lookupId p (preds i)
evalF i val (MPred p ts) = p_sem $ map (evalT i val) ts
  where
    p_sem = lookupId p (mpreds i)
...
evalF i val (Forall var f) = aggrA (universe i) predicate
  where
    predicate a = evalF i (Map.insert var a val) f
evalF i val (Exists var f) = aggrE (universe i) predicate
  where
    predicate a = evalF i (Map.insert var a val) f
evalF i val (Comp var m ts f) =
  -- var:=m(ts)(f)
  do
    a <- m_sem $ map (evalT i val) ts
    evalF i (Map.insert var a val) f
  where
    m_sem = lookupId m (mfuncs i)
...
sample (label, set) =
  do
    sample <- sampleUniform $ SM.toList set
    ...
    { universe = uniformUniverseW,
      funcs =
        Map.fromList
          [ ( "humid_detector",
              \[v] -> case v of
                Int d -> Double (humid_detector d)
                _ -> Double 0.5
            ),
            ( "temperature_predictor",
              \[v] -> case v of
                Int d -> Pair (temperature_predictor d)
                _ -> Pair (0.0, 2.0)
            ),
            ("data1", \_ -> Int 1),
            ("0", \_ -> Int 0),
      ...
      mfuncs =
        Map.fromList
          [ ("bernoulli", \[Double d] -> fmap b2UW $ bernoulli d),
...
weatherSen1 =
  Comp
    "h"
    "bernoulli"
    [Appl "humid_detector" [Appl "data1" []]]
    ( Comp
        "t"
        "normal"
        [Appl "temperature_predictor" [Appl "data1" []]]
        ( Or
            ( And
                (Pred "==" [Var "h", Appl "1" []])
                (Pred "<" [Var "t", Appl "0.0" []])
...
weatherBody =
  Comp
    "h"
    "bernoulli"
    [Appl "humid_detector" [Var "d"]]
    ( Comp
        "t"
        "normal"
        [Appl "temperature_predictor" [Var "d"]]
        ( Or
            ( And
                (Pred "==" [Var "h", Appl "1" []])
                (Pred "<" [Var "t", Appl "0.0" []])
