...
\begin{enumerate*}
  ...
  \item An object $\Omega \in \ObC$ acting as truth basis (similar to the basis of a vector space)
  \item an aggregated 2CMon-Lat $\mathcal R$ internal to $\mathcal{C}$ on the truth space $\mathcal T\Omega $
\end{enumerate*}
...
two disjoint $S^{*}$-sorted families: $\P = (\P_{w})_{w \in S^{*}}$  and $\MP = (\MP_{w})_{w \in S^{*}}$ of {\em predicate symbols} and {\em computational predicate symbols}. \ednote{ \textbf{1Fh6}: "Definition 5: The families of function symbols (etc) are confusing. Is the point that there is a single function symbol for each combination of sorts? The subscript does not seem to be used afterwards."}
