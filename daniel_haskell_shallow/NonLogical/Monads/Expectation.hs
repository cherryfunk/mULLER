...
------------------------------------------------------
------------------------------------------------------
-- CANONICAL ISOMORPHISMS FOR PROBABILITY MONADS
-- p(\nu) = \mathbb{E}_\nu[I_{\{True\}}] = \nu(\{True\})
------------------------------------------------------

-- | Canonical isomorphism p_G : Giry(Bool) -> [0,1]
probGiry :: Giry Bool -> Double
probGiry m = expect Booleans m (\b -> if b then 1.0 else 0.0)

-- | Canonical isomorphism p_D : Dist(Bool) -> [0,1]
probDist :: Dist Bool -> Double
probDist m = expect Booleans m (\b -> if b then 1.0 else 0.0)

------------------------------------------------------
------------------------------------------------------
-- GIRY INSTANCE
-- For EACH OBJECT of DATA, we define how to integrate.
...
instance HasExpectation Giry where
  ...
  expect obj (Bind m k) f = evalBind m (\x -> expect obj (k x) f)
  -- 1. Reals (R) -- Categorical on Reals is an error. Use continuous constructors.
  expect Reals (Categorical _) _ = error "Giry: Categorical on Reals -- use Normal, Uniform, GenericCont, etc."
  expect Reals giry f = evalContinuous giry f
  -- 2. Integers (Z) -- countably infinite: chained convergence strategy.
  expect Integers (Categorical xs) f = chainedDiscreteStrategy xs f
  -- 3. Strings (countably infinite, like Integers).
  expect Strings (Categorical xs) f = chainedDiscreteStrategy xs f
  -- 4. Booleans ({True, False}) -- finite set with exactly 2 elements.
  expect Booleans (Categorical xs) f = sum [p * f x | (x, p) <- xs]
  -- 5. Finite sets ({x_1, ..., x_n}) -- finite direct summation.
  expect (Finite _) (Categorical xs) f = sum [p * f x | (x, p) <- xs]
  -- 6. Unit ({()}) -- trivial: only one element.
  expect Unit _ f = f ()
  -- 7. Products (A x B) -- Categorical on products: finite summation.
  --    For continuous products (R x R), one would need iterated quadrature (Fubini).
  --    Currently, products are constructed via Bind chains which decompose them.
  expect (Prod _ _) (Categorical xs) f = sum [p * f x | (x, p) <- xs]
  -- DisUniform: desugar to Categorical with uniform weights
  expect obj (DisUniform xs) f = expect obj (Categorical [(x, 1.0 / fromIntegral (length xs)) | x <- xs]) f
  -- Poisson: PMF-weighted sum over $\mathbb{N}$ with convergence
  expect _ (Poisson lambda) f = chainedDiscreteStrategy [(k, probability (Poi.poisson lambda) k) | k <- [0 ..]] f
  -- Geometric: PMF-weighted sum over $\mathbb{N}_0$ with convergence
  expect _ (Geometric p) f = chainedDiscreteStrategy [(k, probability (Geo.geometric0 p) k) | k <- [0 ..]] f

------------------------------------------------------
-- DIST INSTANCE
-- Dist always carries finite support -- direct weighted summation.
-- The DATA object is not needed computationally, but is part of the interface.
------------------------------------------------------

instance HasExpectation Dist where
  expect :: DATA a -> Dist a -> (a -> Double) -> Double
  expect _ (Dist xs) f = sum [p * f x | (x, p) <- xs]
...
evalBind (DisUniform xs) f = evalBind (Categorical [(x, 1.0 / fromIntegral (length xs)) | x <- xs]) f
