...
def bind[T, S](monad: Monad[T], kleisli_function: Callable[[T], Monad[S]]) -> Monad[S]:
    """Applies 'kleisli_function' to the value inside 'monad', returning a new monad value but with proper typings."""
    return cast(Monad[S], monad.bind(kleisli_function))  # pyright: ignore[reportArgumentType]


def bind_T[A, B, T: Monad](monad: T, m1: Monad[A], kleisli_function: Callable[[A], Monad[B]]) -> T:
...
def fmap[T, S](monad: Monad[T], function: Callable[[T], S]) -> Monad[S]:
    """Applies 'function' to the value inside 'monad', returning a new monad value but with proper typings."""
