...
class TestDistributionMonad(unittest.TestCase):
    ...
    def assertDistributionEqual(self, dist1: Prob, dist2: Prob, msg=None):
        ...
        self.assertEqual(set(dist1.value.keys()), set(dist2.value.keys()), 
                        msg=f"Distribution keys differ: {msg}")
        for key in dist1.value:
            self.assertAlmostEqualFloat(dist1.value[key], dist2.value[key], 
    ...
    def test_monad_laws(self):
        ...
        a = "test"
        f = lambda x: Prob({x + "_mapped": 0.6, x + "_other": 0.4})
        ...
        g = lambda x: Prob({x + "_g1": 0.2, x + "_g2": 0.8})
    ...
    def test_complex_composition(self):
        ...
        result = (coin
                 .bind(lambda outcome: Prob({outcome + "_round1": 1.0}))
        ...
        self.assertAlmostEqualFloat(result.value["heads_round2_final"], expected_heads_final)
        self.assertAlmostEqualFloat(result.value["tails_round2_final"], expected_tails_final)
