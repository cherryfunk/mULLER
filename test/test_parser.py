...
from muller.nesy_framework import (
    FalseFormula, Predicate, Term, TrueFormula, Negation, Variable, 
    Conjunction, Disjunction, Implication, UniversalQuantification, 
    ExistentialQuantification, FunctionApplication, Computation, MonadicPredicate
)
...
class ParserTestCase(TestCase):
    ...
    def test_predicate_with_function_args(self):
        ...
        self.assertEqual(f, Predicate("taller", [Variable("X"), FunctionApplication("father", [Variable("X")])]))
    ...
    def test_nested_function_term(self):
        ...
        self.assertIsInstance(f, Predicate)
        predicate = cast(Predicate, f)
        expected_func = FunctionApplication("father", [FunctionApplication("mother", [Variable("X")])])
        self.assertEqual(predicate.arguments[0], expected_func)
    ...
    def test_monadic_predicate(self):
        ...
        self.assertEqual(f, MonadicPredicate("probability", [Variable("X")]))    # Test logical connectives
    ...
    def test_universal_quantification(self):
        ...
        self.assertEqual(f, UniversalQuantification("X", Predicate("human", [Variable("X")])))
    
    def test_existential_quantification(self):
        f = parse("exists X human(X)")
        self.assertEqual(f, ExistentialQuantification("X", Predicate("human", [Variable("X")])))
    ...
    def test_nested_quantifiers(self):
        ...
        expected = UniversalQuantification(
            ...
            ExistentialQuantification(
                "Y", 
                Predicate("likes", [Variable("X"), Variable("Y")])
            )
        )
        self.assertEqual(f, expected)
    ...
    def test_computation_in_quantifier(self):
        ...
        expected = ExistentialQuantification(
            "X",
            Computation("X", "transform", [Variable("Y")], Predicate("valid", [Variable("X")]))
        )
        self.assertEqual(f, expected)
    ...
    def test_multiple_property_access(self):
        ...
        expected = Predicate(
            "human", 
            [FunctionApplication("city", [FunctionApplication("address", [Variable("Person")])])]
        )
        self.assertEqual(f, expected)
    ...
    def test_complex_function_names(self):
        ...
        expected = Predicate("human", [FunctionApplication("best friend", [Variable("X")])])
        self.assertEqual(f, expected)
    
    def test_complex_monadic_predicate(self):
        f = parse("$'neural network probability'(X)")
        self.assertEqual(f, MonadicPredicate("neural network probability", [Variable("X")]))
    ...
    def test_numeric_function_multiple_args(self):
        ...
        expected = Predicate("process", [FunctionApplication("123", [Variable("X"), Variable("Y")])])
        self.assertEqual(f, expected)
    
    def test_mixed_numeric_and_text_args(self):
        f = parse("relation(john, 456, mary)")
        expected = Predicate("relation", [
